<?php
// Establecer la conexión con la base de datos
$db = mysqli_connect('localhost', 'root', 'root') or 
    die ('Unable to connect. Check your connection parameters.');
// Seleccionar la base de datos
mysqli_select_db($db,'moviesite', ) or die(mysqli_error($db));

// Verificar si la variable 'do' no está establecida o no es igual a 1
if (!isset($_GET['do']) || $_GET['do'] != 1) {
    // Mostrar un mensaje de confirmación de eliminación dependiendo del tipo
    switch ($_GET['type']) {
        case 'movie':
            echo 'Are you sure you want to delete this movie?<br/>';
            break;
        case 'people':
            echo 'Are you sure you want to delete this person?<br/>';
            break;
    } 
    // Mostrar enlaces para confirmar o cancelar la eliminación
    echo '<a href="' . $_SERVER['REQUEST_URI'] . '&do=1">yes</a> '; 
    echo 'or <a href="admin.php">no</a>';
} else {
    // Si la confirmación está establecida (do=1), proceder con la eliminación
    switch ($_GET['type']) {
        case 'people':
            // Actualizar las películas que tenían a esta persona como actor principal
            $query = 'UPDATE movie SET
                    movie_leadactor = 0 
                WHERE
                    movie_leadactor = ' . $_GET['id'];
            $result = mysqli_query($db, $query) or die(mysqli_error($db));

            // Eliminar la persona de la base de datos
            $query = 'DELETE FROM people 
                WHERE
                    people_id = ' . $_GET['id'];
            $result = mysqli_query($db, $query) or die(mysqli_error($db));
?>
<p style="text-align: center;">Your person has been deleted.
<a href="admin.php">Return to Index</a></p>
<?php
            break;
        case 'movie':
            // Eliminar la película de la base de datos
            $query = 'DELETE FROM movie 
                WHERE
                    movie_id = ' . $_GET['id'];
            $result = mysqli_query($db, $query) or die(mysqli_error($db));
?>
<p style="text-align: center;">Your movie has been deleted.
<a href="admin.php">Return to Index</a></p>
<?php
            break;
    }
}
?>
