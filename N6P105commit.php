...
case 'add':
    ...
    case 'movie':
        ...
        if (empty($error)) {
            ...
            $errorGitano = join('<br/>', array_map('urlencode', $error));
            header('Location:N6P104movie.php?action=add' . '&error=' . $errorGitano);
          }
        break;
        case 'people':
            $error = array();
            ...
            // Validar que al menos uno de los roles (actor o director) esté marcado
            ...
            if (empty($error)) {
                ...
                $errorGitano = join('<br/>', array_map('urlencode', $error));
                header('Location:people.php?action=add' . '&error=' . $errorGitano);
            }
            break;
    }
    break;
case 'edit':
    ...
    case 'movie':
        ...
        if (empty($error)) {
            ...
            $errorGitano = join('<br/>', array_map('urlencode', $error));
            header('Location:N6P104movie.php?action=edit&id=' . $_POST['movie_id'] . '&error=' . $errorGitano);
          }
        break;
        case 'people':
            $error = array();
            ...
            if (empty($error)) {
                ...
                $errorGitano = join('<br/>', array_map('urlencode', $error));
                header('Location:people.php?action=edit&id=' . $_POST['people_id'] . '&error=' . $errorGitano);
            }
            break;
    }
    break;
